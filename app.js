...
function renderTankModule(){
  ...
  $("#tankHistory").innerHTML=tankRecords.map((r,index)=>editingTankRecordIndex===index?`<tr class="tank-edit-row"><td><input data-tank-edit="time" type="time" value="${r.time||""}"></td><td><input data-tank-edit="suctionPsi" inputmode="decimal" value="${r.suctionPsi??0}"></td><td><input data-tank-edit="batchEquivalent" value="${r.batchEquivalent||""}"></td><td><input data-tank-edit="product" value="${escapeHtml(productForRecord(r))}"></td><td><input data-tank-edit="tank" value="${r.tank||""}"></td><td><input data-tank-edit="levelM" inputmode="decimal" value="${r.levelM}"></td><td><input data-tank-edit="gallons" inputmode="decimal" value="${r.gallons}"></td><td><input data-tank-edit="receivedBbl" inputmode="decimal" value="${r.receivedBbl}"></td><td><input data-tank-edit="flowBph" inputmode="decimal" value="${r.flowBph}"></td><td><input data-tank-edit="accumulatedBbl" inputmode="decimal" value="${r.accumulatedBbl}"></td><td class="tank-row-actions"><button class="history-action save" data-save-tank-record="${index}" type="button">Guardar</button><button class="history-action" data-cancel-tank-edit type="button">Cancelar</button></td></tr>`:`<tr><td>${String(r.time||"").replace(":","h")}</td><td>${r.suctionPsi==null?"—":`${fmt(r.suctionPsi,2)} PSI`}</td><td>${r.batchEquivalent||"—"}</td><td>${escapeHtml(productForRecord(r))}</td><td>TP-${String(r.tank||"").padStart(2,"0")}</td><td>${fmt(r.levelM,3)} m</td><td>${fmt(r.gallons)} GLS</td><td>${fmt(r.receivedBbl)} BBL</td><td>${fmt(r.flowBph)} BBL/H</td><td>${fmt(r.accumulatedBbl)} BBL</td><td><button class="history-action" data-edit-tank-record="${index}" type="button">Editar</button></td></tr>`).join("");
