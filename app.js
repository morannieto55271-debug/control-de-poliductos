...
function renderTankModule(){
  ...
  $("#tankHistory").innerHTML=tankRecords.map((r,index)=>editingTankRecordIndex===index?`<tr class="tank-edit-row"><td><input data-tank-edit="time" type="time" value="${r.time||""}"></td><td><input data-tank-edit="suctionPsi" inputmode="numeric" value="${Math.round(r.suctionPsi??0)}"></td><td><input data-tank-edit="batchEquivalent" value="${r.batchEquivalent||""}"></td><td><input data-tank-edit="product" value="${escapeHtml(productForRecord(r))}"></td><td><input data-tank-edit="tank" value="${r.tank||""}"></td><td><input data-tank-edit="levelM" inputmode="decimal" value="${r.levelM}"></td><td><input data-tank-edit="gallons" inputmode="decimal" value="${r.gallons}"></td><td><input data-tank-edit="receivedBbl" inputmode="decimal" value="${r.receivedBbl}"></td><td><input data-tank-edit="flowBph" inputmode="decimal" value="${r.flowBph}"></td><td><input data-tank-edit="accumulatedBbl" inputmode="decimal" value="${r.accumulatedBbl}"></td><td><textarea data-tank-edit="observation" rows="2">${escapeHtml(r.observation||"")}</textarea></td><td class="tank-row-actions"><button class="history-action save" data-save-tank-record="${index}" type="button">Guardar</button><button class="history-action" data-cancel-tank-edit type="button">Cancelar</button></td></tr>`:`<tr><td>${String(r.time||"").replace(":","h")}</td><td>${r.suctionPsi==null?"—":`${fmt(Math.round(r.suctionPsi))} PSI`}</td><td>${r.batchEquivalent||"—"}</td><td>${escapeHtml(productForRecord(r))}</td><td>TP-${String(r.tank||"").padStart(2,"0")}</td><td>${fmt(r.levelM,3)} m</td><td>${fmt(r.gallons)} GLS</td><td>${fmt(r.receivedBbl)} BBL</td><td>${fmt(r.flowBph)} BBL/H</td><td>${fmt(r.accumulatedBbl)} BBL</td><td class="tank-observation-cell">${escapeHtml(r.observation||"—")}</td><td><button class="history-action" data-edit-tank-record="${index}" type="button">Editar</button></td></tr>`).join("");
...
const persistedFieldIds=["tankSelect","batchEquivalentInput","initialTankAccumulated","initialTankTime","tankTime","suctionPsiInput","tankObservationInput","initialLevelMeters","initialLevelCentimeters","initialLevelMillimeters","levelMeters","levelCentimeters","levelMillimeters","stopReason","operationTime","operationObservation"];
...
document.addEventListener("click",e=>{
  ...
  if(save){
    ...
    if(record){Object.assign(record,{time:read("time")||record.time,suctionPsi:Math.round(number("suctionPsi")),batchEquivalent:read("batchEquivalent").trim().toUpperCase(),product:read("product").trim().toUpperCase(),tank:tank||record.tank,levelM:number("levelM"),gallons:number("gallons"),receivedBbl:number("receivedBbl"),flowBph:number("flowBph"),accumulatedBbl:number("accumulatedBbl"),observation:read("observation").trim().toUpperCase()});record.barrels=record.gallons/42;if(index===tankRecords.length-1){forecastFlow=record.flowBph;flowManuallyEdited=false}}
...
["#tankSelect","#batchEquivalentInput","#initialLevelMeters","#initialLevelCentimeters","#initialLevelMillimeters","#levelMeters","#levelCentimeters","#levelMillimeters","#initialTankTime","#tankTime","#initialTankAccumulated"].forEach(s=>$(s).addEventListener("input",renderTankModule));
...
$("#registerTankLevel").addEventListener("click",()=>{
  ...
  $("#initialTankTime").value=time;$("#tankTime").value=addOneHour(time);$("#tankObservationInput").value="";["Meters","Centimeters","Millimeters"].forEach(part=>{$(`#initialLevel${part}`).value=$(`#level${part}`).value});render();message.textContent=confirmation;message.className="transfer-message success";
});
