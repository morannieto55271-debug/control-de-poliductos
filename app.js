...
function renderTankModule(){
  ...
  const calc=tankCalculation(),initial=safeNumber($("#initialTankAccumulated")?.value),accumulated=initial+tankRecords.reduce((s,r)=>s+r.receivedBbl,0),previous=calc.valid?[...tankRecords].reverse().find(r=>r.tank===calc.tank):null,received=calc.valid&&previous?Math.max(0,calc.gallons-previous.gallons)/42:0,hours=previous&&$("#tankTime")?.value?elapsedHours(previous.time,$("#tankTime").value):0,flow=hours?received/hours:0;
  $("#tankAccumulatedDisplay").innerHTML=`${fmt(accumulated)} <small>BBL</small>`;$("#tankReceivedDisplay").innerHTML=`${fmt(received)} <small>BBL</small>`;$("#tankFlowDisplay").innerHTML=`${fmt(flow)} <small>BBL/H</small>`;$("#tankHistoryEmpty").hidden=tankRecords.length>0;
  let running=initial;
...
$("#registerTankLevel").addEventListener("click",()=>{
  ...
  const previous=[...tankRecords].reverse().find(r=>r.tank===calc.tank),receivedBbl=previous?Math.max(0,calc.gallons-previous.gallons)/42:0,hours=previous?elapsedHours(previous.time,time):0,flowBph=hours?receivedBbl/hours:0,completed=applyTransferredVolume(receivedBbl);
